/**
 * scene.js — Ana Sahne Yönetimi (Split-Screen)
 *
 * Angel's Interactive Computer Graphics, 7. Baskı uyumlu:
 *  - lookAt(), perspective() → Bölüm 5 (Viewing)
 *  - rotate(), translate(), scalem() → Bölüm 4 (Transformations)
 *  - Phong Aydınlatma Modeli → Bölüm 6 (Lighting)
 *  - flatten() → GPU veri transferi
 *
 * Özellikler:
 *  - SOL ekran: IMU arabasını takip eden kamera
 *  - SAĞ ekran: WASD arabasını takip eden kamera
 *  - IMU: yunuslama(pitch)=ileri/geri, sapma(yaw)=dönüş (eşik bazlı)
 *  - WASD: W=ileri, S=fren/geri, A=sol, D=sağ
 *  - Güneş/Ay sistemi (mouse tekerleği, 0–180°)
 *  - Düz yarış alanı (400×20 birim)
 */
...
// sapma     >  8° → sağ    (D)
// sapma     < -8° → sol    (A)
var TRACK_RADIUS = 60.0;  // Pist merkez yarıçapı

// Arabalar: iç/dış şerit, başlangıç çizgisinde yan yana
// Pist alt noktası (θ=π/2) → teğet yönü = (-1,0,0) → yaw=-90°
var imuCar = {
...
/**
 * Dairesel yarış pisti — halka şeklinde oval
 * TRACK_RADIUS: pistin merkez yarıçapı
 * Yol genişliği: 18 birim, kaldırım: 3 birim
 */
...
/**
 * Güneş/Ay için UV-Küre geometrisi — stride: pos(3)+normal(3)+uv(2) = 8 float
 */
...
/**
 * Başlangıç çizgisi — pist alt noktasında (x≈0) içten dışa uzanan şerit
 * Damalı bayrak texture ile kaplanmış.
 */
function buildStartLineGeometry() {
  ...
  // Quad: x ∈ [-hw, hw], z ∈ [Ri, Ro]
  // UV u → z (iç→dış track width), v → x (derinlik)
  var v = [
    -hw, y, Ri, 0, 1, 0, 0, 0,
    hw, y, Ri, 0, 1, 0, 0, 1,
    hw, y, Ro, 0, 1, 0, 1, 1,
...
/**
 * Araba geometrisi: gövde + kabin + spoiler + 4 tekerlek + farlar
 * Angel 7th Ed. Ch.4: hiyerarşik dönüşüm matrisleri
 */
...
/**
 * Tekerlek silindiri — Z ekseni etrafında döner (arabanın yan yönü)
 * offset = [cx, cy, cz]: silindir merkezi
 */
...
// ════════════════════════════════════════════════════════════════
// RENDER — SPLIT-SCREEN
// Sol: IMU Arabası Kamerası (🔵)
// Sağ: WASD Arabası Kamerası (🔴)
// ════════════════════════════════════════════════════════════════

var fpsFrames = 0, fpsSec = 0;

/**
 * Tüm sahneyi belirtilen view+proj matrisleri ile çizer.
 * Her viewport için ayrı ayrı çağrılır.
 */
...
function drawCar(bufs, viewM, projM, sunPos, nBlend, cx, cy, cz, yaw, roll, tex) {
  gl.useProgram(progMain);
  // Angel 7th Ed. Ch.4: Hiyerarşik dönüşler
  // rotate(-90, Y): Araba gövdesini +Z yönüne çevirir (far'lar ileri bakar)
  // rotate(yaw, Y): Dünya yönü dönüşü
  // rotate(roll, X): Görsel yan eğim
  var carModel = mult(
...
/**
 * Ana shader uniform'ları ayarla — Angel 7th Ed. Ch.6 Phong aydınlatma
 */
